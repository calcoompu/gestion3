...
$producto = [
    ...
    'stock' => 0, 'stock_minimo' => 1, 'precio_venta' => 0, 'precio_compra' => 0, 'imagen' => ''
];
$errores = [];
$mensaje_exito = '';

// Variables para el menú ( badges y datos )
$total_clientes = 0; $clientes_nuevos = 0; $pedidos_pendientes = 0; $pedidos_hoy = 0;
$facturas_pendientes = 0; $monto_pendiente = 0; $ingresos_mes = 0; $compras_pendientes = 0;
$tablas_existentes = [];
$categorias = [];
$lugares = [];

try {
    // Se establece una única conexión a la BD para toda la página
    $pdo = conectarDB();
    ...
    // --- Lógica del formulario (utiliza la misma conexión $pdo) ---
    if ($es_edicion) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errores)) {
            if ($es_edicion) {
                $sql = "UPDATE productos SET codigo = ?, nombre = ?, descripcion = ?, categoria_id = ?, lugar_id = ?, stock = ?, stock_minimo = ?, precio_venta = ?, precio_compra = ?, imagen = ?, fecha_modificacion = NOW() WHERE id = ?";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$datos['codigo'], $datos['nombre'], $datos['descripcion'], $datos['categoria_id'], $datos['lugar_id'], $datos['stock'], $datos['stock_minimo'], $datos['precio_venta'], $datos['precio_compra'], $imagen_path, $producto_id]);
                $mensaje_exito = 'Producto actualizado correctamente';
            } else {
                $sql = "INSERT INTO productos (codigo, nombre, descripcion, categoria_id, lugar_id, stock, stock_minimo, precio_venta, precio_compra, imagen, activo, fecha_creacion, fecha_modificacion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW(), NOW())";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$datos['codigo'], $datos['nombre'], $datos['descripcion'], $datos['categoria_id'], $datos['lugar_id'], $datos['stock'], $datos['stock_minimo'], $datos['precio_venta'], $datos['precio_compra'], $imagen_path]);
    ...
    // Catch unificado para cualquier error (menú o formulario)
    $errores[] = 'Error del sistema: ' . $e->getMessage();
    // Se asegura de que todas las variables tengan valores por defecto para evitar errores en la vista
    $total_clientes = 0; $clientes_nuevos = 0; $pedidos_pendientes = 0; $pedidos_hoy = 0;
    $facturas_pendientes = 0; $monto_pendiente = 0; $ingresos_mes = 0; $compras_pendientes = 0;
    $tablas_existentes = []; $categorias = []; $lugares = [];
}

$pageTitle = ($es_edicion ? 'Editar' : 'Nuevo') . " Producto - " . SISTEMA_NOMBRE;
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        /* Estilos del Navbar (idénticos a productos.php) */
        .navbar-custom { 
        ...
        .navbar-custom .navbar-nav .nav-link { 
            color: white !important; font-weight: 500; transition: all 0.3s ease;
            margin: 0 2px; border-radius: 5px; padding: 8px 12px !important; font-size: 0.95rem;
        }
        ...
        .image-preview { 
            max-width: 200px; max-height: 200px; border: 2px dashed #0d6efd; border-radius: 8px; 
            padding: 15px; text-align: center; cursor: pointer; transition: all 0.3s ease; 
            background: rgba(13, 110, 253, 0.05); 
        }
        .image-preview:hover { border-color: #0b5ed7; background: rgba(13, 110, 253, 0.1); }
        .image-preview img { max-width: 100%; max-height: 150px; border-radius: 4px; }
    </style>
</head>
<body>
    <!-- Navbar Superior (idéntico al de productos.php) -->
        ...
        <div class="form-body">
            ...
            <form method="POST" enctype="multipart/form-data" id="formProducto">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control" id="precio_compra" name="precio_compra" value="<?php echo $producto['precio_compra']; ?>" step="0.01" min="0" required>
                        </div>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="image-preview" onclick="document.getElementById('imagen').click()">
                            <?php if (!empty($producto['imagen']) && file_exists('../../' . $producto['imagen'])): ?>
                                <img src="../../<?php echo htmlspecialchars($producto['imagen']); ?>?v=<?php echo time(); ?>" alt="Imagen del producto">
                            <?php else: ?>
                ...
                <div class="text-center">
                    <button type="submit" class="btn btn-primary btn-lg px-5"><i class="bi bi-save me-2"></i><?php echo $es_edicion ? 'Actualizar' : 'Guardar'; ?> Producto</button>
    ...
    <!-- Modales (sin cambios) -->
    <div class="modal fade" id="modalCategoria" tabindex="-1"> ... </div>
    <div class="modal fade" id="modalLugar" tabindex="-1"> ... </div>
    ...
    <script>
        ...
        // ... El resto del script de los modales sigue igual ...
    </script>
</body>
</html>