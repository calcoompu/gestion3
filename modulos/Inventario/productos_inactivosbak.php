...
$total_clientes = 0; // Renombrado de $total_clientes_menu para consistencia
$clientes_nuevos = 0;
$pedidos_pendientes = 0; // Se calcula pero el badge no se mostrará aquí
// $pedidos_hoy = 0; // No se usa en el navbar de productos.php
$facturas_pendientes = 0;
// $monto_pendiente = 0; // No se usa en el navbar de productos.php
// $ingresos_mes = 0; // No se usa en el navbar de productos.php
...
try {
    ...
    $stmt_tables_menu = $pdo_menu->query("SHOW TABLES"); // Renombrado para evitar conflicto
    if($stmt_tables_menu){
    ...
    if (in_array('clientes', $tablas_existentes)) {
        // Usando la lógica de productos.php para $total_clientes
    ...
    error_log("Error al cargar datos para el menú en productos_inactivos.php: " . $e->getMessage());
...
if (!in_array($orden_campo, $campos_validos)) {
    $orden_campo = 'fecha_modificacion';
}
...
try {
    ...
    $sql_count = "SELECT COUNT(*) 
                  ...
                  WHERE $where_clause";
    $stmt_count = $pdo->prepare($sql_count); // Renombrado para evitar conflicto
    ...
    $sql_productos = "SELECT p.*, 
            ...
            LIMIT $registros_por_pagina OFFSET $offset"; // Renombrado para evitar conflicto
    $stmt_productos = $pdo->prepare($sql_productos); // Renombrado para evitar conflicto
    ...
    $stmt_cat = $pdo->query($sql_categorias); // Renombrado
    $categorias = $stmt_cat ? $stmt_cat->fetchAll() : [];
    
    $sql_lugares = "SELECT id, nombre FROM lugares WHERE activo = 1 ORDER BY nombre";
    $stmt_lug = $pdo->query($sql_lugares); // Renombrado
    $lugares = $stmt_lug ? $stmt_lug->fetchAll() : [];
    
} catch (Exception $e) {
    $error_mensaje = "Error al cargar productos inactivos: " . $e->getMessage();
    $productos = []; // Asegurar que estén definidas en caso de error
    $categorias = [];
    $lugares = [];
    $total_registros = 0;
    $total_paginas = 0;
...
function obtenerIconoOrden($campo, $orden_actual, $direccion_actual_param) { // Renombrado parámetro
    if ($campo !== $orden_actual) {
        return '<i class="bi bi-arrow-down-up text-muted"></i>';
    }
    return $direccion_actual_param === 'ASC' ? '<i class="bi bi-arrow-up text-primary"></i>' : '<i class="bi bi-arrow-down text-primary"></i>';
}
?>
...
<head>
    ...
    <style>
        ...
        .main-container {
            ...
            /* padding-top: 56px;  Ya no es necesario si el navbar es sticky-top y no fixed-top */
        }
        
        .fixed-header { /* Esta clase se usa para el header debajo del navbar */
            flex-shrink: 0;
            background: #f8f9fa;
            padding: 20px;
        ...
        .content-area { /* Esta clase se usa para el contenedor de la tabla */
            flex: 1;
            display: flex;
            flex-direction: column;
            overflow: hidden;
            padding: 0 20px 20px 20px; /* Añadido padding inferior */
        }
        ...
        .table th {
            ...
            position: sticky;
            top: 0;
            z-index: 10; /* Asegurar que esté sobre el contenido al hacer scroll */
        }
        ...
        .pagination-container { /* Estilo para el contenedor de paginación dentro de la tabla */
            flex-shrink: 0;
            /* background: white; Ya está en .card-footer */
...
<body>
            ...
            <div class="collapse navbar-collapse" id="navbarMain">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="../../modulos/admin/configuracion_sistema.php"><i class="bi bi-sliders me-2"></i>Configuración General</a></li>
                            <li><a class="dropdown-item" href="../../modulos/admin/usuarios.php"><i class="bi bi-people-fill me-2"></i>Gestión de Usuarios</a></li>
                            <li><a class="dropdown-item" href="../../modulos/admin/logs_sistema.php"><i class="bi bi-journal-text me-2"></i>Logs del Sistema</a></li>
                        </ul>
                    </li>
                    <?php endif; ?>
                </ul>
        ...
        <div class="fixed-header">
            ...
            <div class="search-section">
                <form method="GET" class="row g-2 align-items-end">
                    ...
                    <div class="col-md-2"><label class="search-label">Categoría</label><select class="form-select" name="categoria"><option value="0">Todas</option><?php foreach ($categorias as $categoria): ?><option value="<?php echo $categoria['id']; ?>" <?php echo $filtro_categoria == $categoria['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($categoria['nombre'], ENT_QUOTES, 'UTF-8'); ?></option><?php endforeach; ?></select></div>
                    <div class="col-md-2"><label class="search-label">Ubicación</label><select class="form-select" name="lugar"><option value="0">Todas</option><?php foreach ($lugares as $lugar): ?><option value="<?php echo $lugar['id']; ?>" <?php echo $filtro_lugar == $lugar['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($lugar['nombre'], ENT_QUOTES, 'UTF-8'); ?></option><?php endforeach; ?></select></div>
                            ...
                            <?php else: ?>
                                <?php foreach ($productos as $producto): ?>
                                    <tr id="producto-<?php echo $producto['id']; ?>">
                                        ...
                                        <td><strong><?php echo htmlspecialchars($producto['nombre'], ENT_QUOTES, 'UTF-8'); ?></strong><?php if (!empty($producto['descripcion'])): ?><br><small class="text-muted"><?php echo htmlspecialchars(mb_substr($producto['descripcion'], 0, 50), ENT_QUOTES, 'UTF-8'); ?>...</small><?php endif; ?></td>
                                        <td><?php if ($producto['categoria_nombre']): ?><span class="badge bg-secondary"><?php echo htmlspecialchars($producto['categoria_nombre'], ENT_QUOTES, 'UTF-8'); ?></span><?php else: ?><span class="text-muted">Sin categoría</span><?php endif; ?></td>
                                        <td><?php if ($producto['lugar_nombre']): ?><span class="badge bg-info"><?php echo htmlspecialchars($producto['lugar_nombre'], ENT_QUOTES, 'UTF-8'); ?></span><?php else: ?><span class="text-muted">Sin ubicación</span><?php endif; ?></td>
    ...
    <script>
        ...
        function mostrarMensaje(mensaje, tipo) {
            ...
            new bootstrap.Alert(alertDiv); // Inicializar para que funcione el dismiss
